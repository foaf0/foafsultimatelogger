...
local connect = {
        {
	        ...
	        ["footer"] = {
                ["text"] = "Made By: Foaf#0001 | Foafs Ultimate Log",
                ["icon_url"] = FoafUltimateLog.Logo,
            },
        }
    }

PerformHttpRequest(FoafUltimateLog.Webhook, function(err, text, headers) end, 'POST', json.encode({username = FoafUltimateLog.ServerName, embeds = connect}), { ['Content-Type'] = 'application/json' })
end)
...
local disconnect = {
    ...
    PerformHttpRequest(FoafUltimateLog.Webhook, function(err, text, headers) end, 'POST', json.encode({username = FoafUltimateLog.ServerName, embeds = disconnect}), { ['Content-Type'] = 'application/json' })
end)
